...
namespace Ludo {
	...
	bool DirectX12VertexBuffer::Init(float* verticies, uint32_t size)
	{
		...
		DirectX12UploadBuffer upBufffer(size);
		upBufffer.UploadData(m_VertexBuffer, verticies);
	...
	bool DirectX12IndexBuffer::Init(uint32_t* indices, uint32_t count)
	{
		...
		DirectX12UploadBuffer upBuffer(size);
		upBuffer.UploadData(m_IndexBuffer, indices);
