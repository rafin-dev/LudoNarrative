...
namespace Ludo {
	...
	// ========== Vertex Buffer ==========
	DirectX11VertexBuffer::DirectX11VertexBuffer(float* verticies, uint32_t size, const BufferLayout& layout, VBUpdateFrequency frequency)
		: m_Layout(layout)
	{
		LD_PROFILE_FUNCTION();
	...
	DirectX11VertexBuffer::DirectX11VertexBuffer(uint32_t size, const BufferLayout& layout, VBUpdateFrequency frequency)
		: m_Layout(layout)
	{
		LD_PROFILE_FUNCTION();
	...
	void DirectX11VertexBuffer::BindImpl(uint32_t slot) const
	{
		UINT stride = m_Layout.GetStride();
		UINT offset = 0;
		DirectX11API::Get()->GetDeviceContext()->IASetVertexBuffers(slot, 1, &m_VertexBuffer, &stride, &offset);
	}
	...
	void DirectX11VertexBuffer::SetData(void* data, uint32_t size)
	{
		...
		LD_CORE_ASSERT(SUCCEEDED(hr), "Failed to map Vertex Buffer");
