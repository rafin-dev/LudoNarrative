...
namespace Ludo {
	...
	bool DirectX11Texture2D::Init(void* data, int channelCount)
	{
		...
		VALIDATE_DX_HRESULT(hr, "Failed to create Texture: {0} [{1}, {2}]", m_Path.string(), m_Width, m_Height);

		hr = device->CreateShaderResourceView(m_Texture, nullptr, &m_ShaderResourceView);
		VALIDATE_DX_HRESULT(hr, "Failed to create Shader Resource View for texture: {0}, [{1}, {2}]", m_Path.string(), m_Width, m_Height);
		...
		VALIDATE_DX_HRESULT(hr, "Failed to create Sampler State for Texture: {0} [{1}, {2}]", m_Path.string(), m_Width, m_Height);

		LD_CORE_TRACE("Loaded Texture: {0} [{1}, {2}]", m_Path.string(), m_Width, m_Height);
	...
	void DirectX11Texture2D::ShutDown()
	{
		...
		LD_CORE_TRACE("Unloaded Texture: {0} [{1}, {2}]", m_Path.string(), m_Width, m_Height);
	}
